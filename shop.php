<?php
  ...
  if(!$log){
	...
	case 'items':
	  ...
	  ";
	  $atrlist='';
	  for($i=0;$i<9;$i++) $atrlist.="$tccellh width=6%>$stat[$i]</td>";
	  ...
	  while($item=mysql_fetch_array($items)){
		...
		$comm="width=30><a href=shop.php?action=buy&id=$item[id]>Buy</a></td>$tccell1s width=50>$preview";
	    }elseif(!$eq[e] && !$item[id]){
		...
		<tr$color>
		$tccell1s $comm</td>
		$tccell2ls><b>$item[name]</b></td>
		$atrlist
		$tccell2rs>$item[coins]</td>
		$tccell2rs>0</td>
	    ";
	  }
	  print $tblend;
