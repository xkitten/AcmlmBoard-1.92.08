<?php 
  ...
  for($i=0;$i<$msgtotal;$i+=$ppp){
    $p++;
    if($p==$page) $pagelinks.=" $p";
    else $pagelinks.=" <a href=private.php?$idparam$viewparam"."page=$p>$p</a>";
  }
  $pmsgs=mysql_query("SELECT p.id,user$from u,date,t.title,msgread,name,sex,powerlevel FROM pmsgs p,pmsgs_text t,users u WHERE user$to=$u AND p.id=pid AND user$from=u.id ORDER BY p.id DESC LIMIT $pmin,$ppp");
  $from[0]=strtoupper($from[0]);
  if(!$view) $viewlink="<a href=private.php?$idparam"."view=sent>View sent messages</a>";
  else $viewlink="<a href=private.php?$idparam>View received messages</a>";
  print "
	$header
  ...
  while($pmsg=mysql_fetch_array($pmsgs)){
    $new=($pmsg[msgread]?'&nbsp;':'<img src=images/new.gif>');
    $uid=$pmsg[u];
