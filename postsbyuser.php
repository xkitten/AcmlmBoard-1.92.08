<?php
  ...
  $postlist="
	$tccellhs width=50>Post id</td>
	$tccellhs width=50>Post#</td>
	$tccellhs width=130>Posted on</td>
	$tccellhs>Thread</td>
  ";
  if($isadmin) $postlist.="$tccellhs width=110>IP address</td>";
  for($i=0;$post=mysql_fetch_array($posts);$i++){
    if($i>=$min and $i<($min+$ppp)){
	if($post[minpower]<=$power or !$post[minpower]) $threadlink="<a href=thread.php?id=$post[thread]>".str_replace('<','&lt',$post[title]).'</a>';
