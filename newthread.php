<?php
  ...
  if($poll){
    ...
    $choices.="Choice $c: $inpt=chtext[$c] SIZE=30 MAXLENGTH=255> &nbsp Color: $inpt=chcolor[$c] SIZE=7 MAXLENGTH=25><br>$inps=paction VALUE=\"Submit changes\">";
  ...
  $form=(!$poll?"
		...
		$tccell1><b>Thread title:</td>$tccell2l>$inpt=subject SIZE=40 MAXLENGTH=100 VALUE=\"". stripslashes($subject) ."\"><tr>
		$tccell1><b>Post:</td>		$tccell2l>".replytoolbar(2)."$txta=message ROWS=20 COLS=$numcols ".replytoolbar(3).">". stripslashes($message) ."</TEXTAREA><tr>
		  $tccell1><b>Options:</b></td>$tccell2l>
  ...
  ":"
		$tccell1><b>Poll icon:</td>	$tccell2l>$posticonlist<tr>
		$tccell1><b>Poll title:</td>	$tccell2l>$inpt=subject SIZE=40 MAXLENGTH=100 VALUE=\"". stripslashes($subject) ."\"><tr>
		$tccell1><b>Question:</td>	$tccell2l>$inpt=question SIZE=60 MAXLENGTH=255 VALUE=\"". stripslashes($question) ."\"><tr>
		$tccell1><b>Briefing:</td>	$tccell2l>$txta=briefing ROWS=2 COLS=$numcols>". stripslashes($briefing) ."</TEXTAREA><tr>
		$tccell1><b>Multi-voting:</td>$tccell2l>$radio=mltvote value=0 $checked0> Disabled &nbsp $radio=mltvote value=1 $checked1> Enabled<tr>
		$tccell1><b>Choices:</td>	$tccell2l>$choices<tr>
		$tccell1><b>Post:</td>		$tccell2l>".replytoolbar(2)."$txta=message ROWS=20 COLS=$numcols ".replytoolbar(3).">". stripslashes($message) ."</TEXTAREA><tr>
		  $tccell1><b>Options:</b></td>$tccell2l>
			$inpc=\"nosmilies\" id=\"nosmilies\" value=\"1\"$nosmilieschk><label for=\"nosmilies\">Disable Smilies</label> - 
			$inpc=\"nolayout\" id=\"nolayout\" value=\"1\"$nolayoutchk><label for=\"nolayout\">Disable Layout</label> - 
		...
		$inps=preview VALUE=\"Preview poll\"></td></FORM>
  ");
  ...
  if($log and $forums[$id][minpowerthread]>$power){
	...
	$tccell1><b>User name:</td>	$tccell2l>$inpt=username VALUE=\"".htmlspecialchars($loguser[name])."\" SIZE=25 MAXLENGTH=25><tr>
	$tccell1><b>Password:</td>	$tccell2l>$inpp=password VALUE=\"".htmlspecialchars($logpassword)."\" SIZE=13 MAXLENGTH=32><tr>
    ";
    print $form;
  }
  print "
